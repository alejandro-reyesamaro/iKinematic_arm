    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_IK_PF_NET.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * IK_PF_NET.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * IK_PF_NET.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * IK_PF_NET (application created by MATLAB Compiler)
        * run_IK_PF_NET.sh (shell script that can be run to 
    ...
    All Platforms:
        * IK_PF_NET.dll (shared library, file extension varies by 
          platform)
        * IK_PF_NET.h (library header file)
        * IK_PF_NET.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * IK_PF_NET.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * IK_PF_NET.xml (documentation files)
        * IK_PF_NET.pdb (program database file - if DEBUG option 
          selected)
        * IK_PF_NET.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * IK_PF_NET.ctf (Component Technology File archive, contains 
          compiled M-code)
        * IK_PF_NET_0_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * IK_PF_NET.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * IK_PF_NET.ctf (Component Technology File archive, contains 
          compiled M-code)
        * IK_PF_NET.xla (Optionally created and placed in
          C:\Documents and Settings\Administrador\Escritorio\My Work\MATH\ParticleFiltering\IK_ParticleFiltering\IK_PF_NET\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
