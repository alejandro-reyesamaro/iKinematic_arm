...
namespace TrapezoidalMap
{
    public class TMSearchGraph
    {
        ...
        private TMANode lower_right_recursivo(TMANode A, TMNode D)
        {
            if (D is TMANode)
            {
            ...
            {
                TMANode T = lower_right_recursivo(A, D.Node_Left);
                if (T == null)
                    T = lower_right_recursivo(A, D.Node_Rigth);
                return T;
            }
        }
        ...
        {//function Delta = followSegment(D, T, S)
            List<TMNode> Delta = new List<TMNode>();
            //% Puntos extremos del segmento.
            TMPoint p = S.Point_1;
            TMPoint q = S.Point_2;
            TMANode A_ = (TMANode)this.root.buscar(p, S);
            Delta.Add(A_);
            int j = 0;
            ...
            while (q.X > A_.Trap.RightP.X)
            {
                ...
                TMANode A_1;
                if (A_.Trap.RightP.Y > S.eval(A_.Trap.RightP.X)) //A_+1 = trapecio lewer-right vecino de A_
                    A_1 = lower_right(A_);
                else //A_+1 = trapecio upper-roght vecino de A_
                    A_1 = upper_right(A_);
                if (A_1 == null) break;
                Delta.Add(A_1);
                A_ = A_1;                
        ...
        public void actualiza_grafo_busqueda(List<TMNode> Delta, TMSegment S)//, out TMNode primero) // el primero ese hay que eliminarlo
        {
            ...
            bool a1_completo = true, a2_completo = true;
            ...
            for (int i = 0; i < Delta.Count; i++)
            {
                ...
                if (i == 0 && S.Point_1.X > T.LeftP.X)
                {
                    ...
                    TMPoint top_rigth_A = new TMPoint(S.Point_1.X, T.Top.eval(S.Point_1.X), T.Top_Left.ID);
                    TMPoint bottom_left_A = T.Bottom_Left;
                    TMPoint bottom_right_A = new TMPoint(S.Point_1.X, T.Bottom.eval(S.Point_1.X), T.LeftP.ID);
                    ...
                    TMTrapezoid A = new TMTrapezoid(leftp_A, rightp_A, top_left_A, top_rigth_A, bottom_left_A, bottom_right_A);
                    ...
                    {                        
                        ...
                        padres = new List<TMNode>(); padres.Add(P);//padre = padre[Delta_node.Node_type];
                ...
                if (i == Delta.Count - 1 && S.Point_2.X < T.RightP.X)
                {
                    ...
                    TMPoint top_left_D = new TMPoint(S.Point_2.X, T.Top.eval(S.Point_2.X), T.Top_Right.ID);
                    TMPoint bottom_right_D = T.Bottom_Right;
                    TMPoint bottom_left_D = new TMPoint(S.Point_2.X, T.Bottom.eval(S.Point_2.X), T.Bottom_Right.ID);
                 
                    TMNode Q = new TMPNode(S.Point_2);
                    TMTrapezoid D = new TMTrapezoid(leftp_D, rightp_D, top_left_D, top_right_D, bottom_left_D, bottom_right_D);
                    ...
                    if (padres != null)
                    {
                        ...
                        padres = new List<TMNode>(); padres.Add(Q); //padre = padre[Delta_node.Node_type]; 
                ...
                if (a1_completo) //(*)
                {
                    ...
                    TMANode A1_node = new TMANode(A1);
                    for(int k = 0; k < a1_padres.Count; k++)
                    {
                        TMNode padre = a1_padres[k];                            
                ...
                if (a2_completo) //(*)
                {
                    ...
                    TMANode A2_node = new TMANode(A2);
                    for (int k = 0; k < a2_padres.Count; k++)
                    {
                        TMNode padre = a2_padres[k];                        
        ...
        {
            List<TMTrapezoid> trapecios = new List<TMTrapezoid>();
            lista_trapecios_exteriores_recursivo(this.root, trapecios);
            return trapecios;
        }
        private void lista_trapecios_exteriores_recursivo(TMNode node, List<TMTrapezoid> trapecios)
        {
            if (node is TMANode)
            {
        ...
        private void lista_trapecios_recursivo(TMNode node, List<TMTrapezoid> trapecios)
        {
            if (node is TMANode)
                trapecios.Add(((TMANode)node).Trap);
            else 
